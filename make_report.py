...
def body(pdf):
    ...
    pdf.body(
        ...
        "설정하고 자동 크롤러를 개발하였습니다. 최종 5,910건을 수집하였으며 "
    ...
    pdf.body(
        ...
        "384차원 벡터로 변환하여 저장하였습니다. 5,910건 전체 임베딩 완료 후 "
    ...
    pdf.tbl(
        ["기능", "상태", "세부 내용"],
        [
            ["국립중앙도서관 Q&A 크롤링",  "완료",    "5,910건, 591페이지 전체 수집"],
            ["PostgreSQL DB 구축",         "완료",    "ai_librarian DB, qa_records 테이블"],
            ["로컬 임베딩 생성",           "완료",    "384차원, 5,910건 전체 적용"],
            ...
            ["질문+답변 결합 임베딩",      "개선완료", "질문+답변(200자) 결합 임베딩으로 교체, 5,910건 재생성"],
            ["한국어 형태소 분석",         "개선완료", "kiwipiepy 기반 morphemes 컬럼 + GIN 인덱스 (5,910건)"],
    ...
    pdf.tbl(
        ...
        [
            ...
            ["crawler/data/",       "keys.json, qa_data.json (5,910건)"],
    ...
    pdf.need_con(
        ...
        "전체 5,910건 재임베딩 수 시간 소요, 모델 파일 수 GB 저장 공간 부담, "
    ...
    pdf.need_con(
        "현재 5,910건은 특정 시점의 스냅샷으로, 시간이 지날수록 최신 사서 답변이 누락되어 "
    ...
    pdf.sub("7.1 프로젝트 요약")
    pdf.body(
        "본 프로젝트는 국립중앙도서관이 '사서문답' 서비스를 통해 축적한 5,910건의 전문 사서 답변 데이터를 "
    ...
    results = [
        ("수집 데이터",     "5,910건 (국립중앙도서관 사서문답 전체, 100% 수집)"),
        ...
        ("텍스트 정제",     "곡선 따옴표·괄호 공백 정규화, 5,910건 전체 재임베딩 완료"),
